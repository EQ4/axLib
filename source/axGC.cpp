...
namespace ax
{
    ...
    ax::FloatRect GC::RectToFloatRect(const ax::Rect& rect)
    {
        return ax::FloatRect(double(rect.position.x), double(rect.position.y),
                             double(rect.size.x), double(rect.size.y));
    }
    
    void GC::SetColor(const double& r, const double& g, const double& b)
    {
        glColor4d(r, b, g, 1.0);
    }
    
    void GC::SetColor(const double& r,
                        const double& g,
                        const double& b,
                        const double& a)
    {
    ...
    void GC::SetColor(const ax::Color& color, const float& alpha)
    {
    ...
    void GC::DrawRoundedRectangleContour(const ax::Rect& rect, const int& radius)
    {
    ...
    void DrawQuarterCircleContourSmooth(GC* gc, const ax::FloatPoint& pos,
                                        const int& radius,
                                        const double& angle,
                                        const int& nSegments)
    {
    ...
    void GC::DrawRoundedRectangleContourSmooth(const ax::Rect& rect,
                                                 const int& radius)
    {
    ...
    void GC::DrawRoundedRectangle(const ax::Rect& rect, const int& radius)
    {
        int r = radius;
    ...
    void GC::DrawRectangleContour(const ax::Rect& rect, float linewidth)
    {
        //    axMatrix4 mview_before(GL_MODELVIEW_MATRIX);
        //    axMatrix4 mview;
        //    mview.Identity().Translate(_win->GetAbsoluteRect().position).Process();
    ...
    void GC::DrawTexture(GLuint texture, const ax::Rect& rect, ax::Color color)
    {
    ...
    void GC::DrawImage(ax::Image* img, const ax::Point& position, double alpha)
    {
    ...
    void GC::DrawImageResize(ax::Image* img, const ax::Point& position, const ax::Size& size, double alpha)
    {
        //	ax::Point pos = position + _win->GetAbsoluteRect().position;
        ax::Point pos = position;
        //pos -= _win->GetScrollDecay();
    ...
    void GC::DrawPartOfImage(ax::Image* img,
    ...
    void GC::DrawPartOfImageResize(ax::Image* img,
                                     const ax::Point& posInImage,
                                     const ax::Size& sizeInImage,
                                     const ax::Rect& rect,
                                     double alpha)
    ...
    void GC::DrawString(ax::Font& font,
    ...
    ax::Point GC::DrawChar(ax::Font& font,
                             const char& key,
                             const ax::Point& pos)
    {
        if(font)
    ...
    //void GC::BlockDrawing(const ax::Rect& rect)
    //{
    ...
    //}
    //
    //void GC::UnBlockDrawing()
    //{
    //    glDisable(GL_SCISSOR_TEST);
    //}
    
    void GC::DrawStringAlignedCenter(ax::Font& font,
                                       const std::string& text,
                                       //const ax::Point& pos,
                                       const ax::Rect& rect)
    {
    ...
    void GC::DrawRectangleColorFade(const ax::Rect& rectangle,
    ...
    void GC::DrawLines(const std::vector<ax::Point>& pts, float width)
    {
    ...
    void GC::DrawSmouthLine(const ax::Point& pt1, const ax::Point& pt2, float width)
    {
        //    glEnable(GL_LINE_SMOOTH);
    ...
    void GC::DrawLine(const ax::Point& pt1, const ax::Point& pt2, float width)
    {
    ...
    void GC::DrawLineColorfade(const ax::Point& pt1, const ax::Point& pt2,
    ...
    void GC::DrawPoint(const ax::Point& pt, const int& size)
    {
    ...
    void GC::DrawLineCubic(const ax::Point& pt1, const ax::Point& pt2)
    {
        // H1(t) = 2t^3 - 3t^2 + 1
        // H2(t) = -2t^3 + 3t^2
        // H3(t) = t^3 - 2t^2 + t
    ...
    void GC::SetLineWidth(const double& width)
    {
        //    glEnable(GL_LINE_SMOOTH);
        //    glEnable(GL_POLYGON_SMOOTH);
        //    glEnable(GL_BLEND);
    ...
    void GC::SetSmoothLine(const int& width)
    {
        glEnable(GL_LINE_SMOOTH);
        glEnable(GL_POLYGON_SMOOTH);
        glHint(GL_LINE_SMOOTH_HINT, GL_NICEST);
    ...
    void GC::DrawCircle(const ax::Point& pos,
                          const double& radius,
                          const int& nSegments)
    {
    ...
    //
    //void GC::DrawBigImageResize(axBigImage* img,
    //	const ax::Point& position,
    //	const ax::Size& size,
    //	double alpha)
    //{
    ...
    void GC::DrawPolygone(const std::vector<ax::Point>& points)
    {
        glBegin(GL_POLYGON);
    ...
    void GC::DrawArrow(const ax::Point& p1, const ax::Point& p2,
    ...
    void GC::DrawTriangleColorFade(const ax::Point& p1,
                                     const ax::Point& p2,
                                     const ax::Point& p3,
                                     const ax::Color& c1,
                                     const ax::Color& c2,
